...
if(!isset($_GET['status'])) {		
    ...
    if($readyToPay AND !empty($response_order)) { 
		...
		echo '<INPUT TYPE="button" value="Oплатить Visa" onclick="document.location = \''.$twpg_url.'\'">';
	}
	 
	if(CSalePaySystemAction::GetParamValue("TWO") == 'Y') {                                                                               // если активны два процессинга, то работаем еще и с Uniteller
	   echo ' <INPUT TYPE="button" onclick="document.forms.uniteller.submit()" value="Oплатить MasterCard">';
	...
	switch ($status) {
				...
				case 'CANCELED':
					echo '<div class="ubr_f">Oплата отменена пользователем</div>';
					break;
					
				case 'DECLINED':
					if(!empty($_GET['desc'])) $desc = '. Причина отказа - '.$_GET['desc'];
					else $desc ='';
					echo '<div class="ubr_f">Oплата отклонена банком'.$desc.'</div>';
					break;

				case '0':
					echo '<div class="ubr_f">Oплата не совершена</div>';                                                                                          //эти два пункта по ёнителлеру
					break;		
					
				case '1':
					echo '<div class="ubr_s">Oплата совершена успешно, ожидайте обработки заказа</div>';
					break;			
